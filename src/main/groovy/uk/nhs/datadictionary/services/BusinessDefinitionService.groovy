...
class BusinessDefinitionService extends DataDictionaryComponentService<Term, NhsDDBusinessDefinition> {
    ...
    NhsDDBusinessDefinition show(UUID versionedFolderId, UUID id, NhsDataDictionaryService nhsDataDictionaryService) {
        ...
        NhsDDBusinessDefinition businessDefinition = initialiseComponent(new NhsDDBusinessDefinition(), businessDefinitionTerm, versionedFolderId, nhsDataDictionaryService)
...
//            convertLinksInDescription(versionedFolderId, businessDefinition.getDescription())
    ...
    Set<Term> getAll(UUID versionedFolderId, NhsDataDictionaryService nhsDataDictionaryService, Boolean includeRetired = false) {
        ...
        List<Term> terms = termCacheableRepository.readAllByTerminologyIdIn([busDefTerminology.id])
        Map<UUID, Term> termsMap = terms.collectEntries {
            [it.id, it]
        }
        List<Metadata> metadata = metadataCacheableRepository.findByMultiFacetAwareItemIdInAndNamespaceAndKey(terms.id, new NhsDDBusinessDefinition().getMetadataNamespace(), "isRetired")
